...
```text
   ...
   (Contextual Evidence)                      │
          ...
          ┌───────────────────────────────────┤
          │                                   │
   Target Maturity (1–7)               Travel Duration
          │                                   │
          ▼                                   │
...
### `apps/web`
Fullstack Next.js 16 application. Manages user interactions, form validation, browser-native camera capture, geocoding and routing integration, and orchestrates the schedule optimization workflow.
...
- **Geoapify Routing API**: Turn-by-turn road routing, distance calculation, and duration estimation by vehicle mode (`motorcycle`, `light_truck`, `medium_truck`, `truck`, `heavy_truck`).
